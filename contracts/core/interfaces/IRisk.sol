...
library RiskLib {
    ...
    function calculateHealth(uint256 collateral, uint256 debt) internal pure returns (uint256) {
        if (debt == 0) return type(uint256).max;
        return (collateral * HEALTH_PRECISION) / debt;
    }

    /// @notice Check if position needs intervention
    function needsIntervention(uint256 healthFactor, uint256 threshold) internal pure returns (bool) {
