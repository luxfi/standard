...
interface IShieldedPool {
    ...
    event ShieldedDeposit(bytes32 indexed commitment, uint256 leafIndex, uint64 timestamp);

    /// @notice Emitted when tokens are withdrawn from shielded pool
    event ShieldedWithdrawal(bytes32 indexed nullifier, address indexed recipient, uint256 relayerFee);

    /// @notice Emitted when shielded transfer occurs
    event ShieldedTransferEvent(bytes32 indexed inputNullifier, bytes32 indexed outputCommitment);
    ...
    function deposit(bytes32 commitment, bytes calldata encryptedAmount) external payable returns (uint256 leafIndex);
    ...
    function withdraw(WithdrawalProof calldata proof) external returns (bool success);
    ...
    function shieldedTransfer(ShieldedTransfer calldata transfer) external returns (uint256 outputLeafIndex);
