...
interface ISettlement {
    ...
    function settleWithYield(bytes32 obligationId, address yieldSource, uint256 amount)
        external
    ...
    function routeYield(address yieldSource, bytes32 obligationId, uint256 percentage) external;
    ...
    function projectSettlementCapacity(address yieldSource, uint256 duration) external view returns (uint256 capacity);

    /// @notice Calculate time to full settlement
    /// @param obligationId Target obligation
    /// @param yieldSource Yield source
    /// @return time Estimated seconds to settlement
    function timeToFullSettlement(bytes32 obligationId, address yieldSource) external view returns (uint256 time);
}
...
library SettlementLib {
    ...
    function isCompatible(SettlementType sType, Monotonicity direction) internal pure returns (bool) {
    ...
    function supportsStreaming(SettlementType sType) internal pure returns (bool) {
        return sType == SettlementType.STREAMING || sType == SettlementType.TRANSMUTATION;
    }
}
