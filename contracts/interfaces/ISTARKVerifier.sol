...
interface ISTARKVerifier {
    ...
    event ReceiptCreated(bytes32 indexed receiptHash, bytes32 indexed rootBefore, bytes32 indexed rootAfter);
    ...
    function verifySTARK(bytes32 programId, bytes calldata proof, bytes calldata publicInputs)
        external
    ...
    function verifyAndCreateReceipt(bytes32 programId, bytes calldata proof, bytes calldata publicInputs)
        external
    ...
    function verifyFRI(FRIParams calldata params, bytes32 commitment, bytes calldata queries)
        external
        view
        returns (bool valid);
    ...
    function friFold(uint256[] calldata layer, uint256 challenge) external view returns (uint256[] memory folded);
...
library STARKLib {
    ...
    function verify(bytes32 programId, bytes memory proof, bytes memory publicInputs)
        internal
        view
        returns (bool valid, bytes32 receiptHash)
    {
        (bool success, bytes memory result) = STARK_VERIFY.staticcall(abi.encode(programId, proof, publicInputs));
    ...
    function fieldOp(uint8 op, uint64 a, uint64 b) internal view returns (uint64 result) {
        (bool success, bytes memory output) = STARK_FIELD_ARITH.staticcall(abi.encodePacked(op, a, b));
        require(success, "STARKLib: field op failed");
        result = abi.decode(output, (uint64));
    }

    /// @notice Call FRI fold precompile
    function friFold(uint256[] memory layer, uint256 challenge) internal view returns (uint256[] memory folded) {
        (bool success, bytes memory output) = STARK_FRI_FOLD.staticcall(abi.encode(layer, challenge));
        require(success, "STARKLib: FRI fold failed");
        folded = abi.decode(output, (uint256[]));
    }

    /// @notice Verify Merkle path using STARK precompile
    function verifyMerkle(bytes32 root, bytes32 leaf, bytes32[] memory path, uint256 index)
        internal
        view
        returns (bool valid)
    {
        (bool success, bytes memory output) = STARK_MERKLE.staticcall(abi.encode(root, leaf, path, index));
