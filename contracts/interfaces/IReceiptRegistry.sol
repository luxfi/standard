...
interface IReceiptRegistry {
    ...
    event ProgramRegistered(bytes32 indexed programId, address indexed registrar, string name, uint32[] proofSystems);

    event ProgramVersionAdded(bytes32 indexed programId, uint32 indexed version, uint32 proofSystem, bytes32 vkHash);

    event ProgramDeprecated(bytes32 indexed programId, uint32 indexed version);

    event ProofSubmitted(
        bytes32 indexed receiptHash, bytes32 indexed programId, bytes32 indexed claimHash, uint32 proofSystem
    );

    event RootUpdated(bytes32 indexed newRoot, bytes32 indexed previousRoot, uint32 blockNumber, uint256 receiptCount);

    event AggregationCreated(bytes32 indexed aggregationRoot, bytes32[] receiptHashes, uint256 count);
    ...
    function addProgramVersion(bytes32 programId, uint32 version, uint32 proofSystem, bytes32 vkHash) external;

    /// @notice Deprecate a program version
    /// @param programId Program to deprecate
    /// @param version Version to deprecate
    function deprecateVersion(bytes32 programId, uint32 version) external;
    ...
    function getProgramVersion(bytes32 programId, uint32 version)
        external
        view
        returns (ProgramVersion memory versionInfo);
    ...
    function submitProof(bytes32 programId, uint32 proofSystem, bytes calldata proof, bytes calldata publicInputs)
        external
    ...
    function submitBatch(BatchVerifyRequest[] calldata requests) external returns (BatchVerifyResult[] memory results);
    ...
    function submitAggregation(bytes32[] calldata parentReceipts, bytes calldata aggregationProof)
        external
        returns (bytes32 aggregationRoot);
    ...
    function getReceiptWithProof(bytes32 receiptHash) external view returns (ReceiptWithProof memory data);
    ...
    function getReceiptsByProgram(bytes32 programId, uint256 offset, uint256 limit)
        external
        view
        returns (bytes32[] memory receiptHashes);
    ...
    function getInclusionProof(bytes32 receiptHash) external view returns (bytes32[] memory proof, bytes32 root);
    ...
    function verifyInclusion(bytes32 receiptHash, bytes32[] calldata proof, bytes32 root)
        external
        view
        returns (bool valid);
    ...
    function exportReceipt(bytes32 receiptHash, uint32 targetProofSystem) external view returns (bytes memory proof);
    ...
    function getGroth16Proof(bytes32 receiptHash)
        external
        view
        returns (bytes memory groth16Proof, bytes memory publicInputs);
...
interface IReceiptVerifier {
    ...
    function verifyReceipt(bytes32 receiptHash, bytes32[] calldata proof, bytes32 root)
        external
        view
        returns (bool valid);
    ...
    /// @param publicInputs Public inputs
    /// @return valid True if proof is valid
    function verifyGroth16(bytes calldata proof, bytes calldata publicInputs) external view returns (bool valid);
}
