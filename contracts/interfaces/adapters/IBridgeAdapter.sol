...
interface IBridgeAdapter {
    ...
    event BridgeCompleted(
        bytes32 indexed bridgeId, address indexed recipient, uint256 dstChainId, address token, uint256 amount
    );
    ...
    function isRouteSupported(uint256 dstChainId, address token) external view returns (bool);

    /// @notice Get route information
    function getRoute(uint256 dstChainId, address token) external view returns (BridgeRoute memory);
    ...
    function bridge(BridgeParams calldata params) external payable returns (bytes32 bridgeId);

    /// @notice Get bridge transaction status
    /// @param bridgeId Bridge transaction ID
    /// @return BridgeStatus struct
    function getStatus(bytes32 bridgeId) external view returns (BridgeStatus memory);
    ...
    function estimateOutput(uint256 dstChainId, address token, uint256 amount) external view returns (uint256 amountOut);

    /// @notice Estimate bridge time
    /// @param dstChainId Destination chain
    /// @return Estimated time in seconds
    function estimateTime(uint256 dstChainId) external view returns (uint256);
}
