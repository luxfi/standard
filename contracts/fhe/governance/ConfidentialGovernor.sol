 ...
 */
abstract contract ConfidentialGovernor is Ownable2Step, TFHEApp {
    ...
    /// @dev    It is set at 100,000, which is 1% of the total supply of the ConfidentialLRC20Votes token.
    ...
    /// @dev    It is set at 400,000, which is 4% of the total supply of the ConfidentialLRC20Votes token.
    ...
    /// @notice ConfidentialLRC20Votes governance token.
    IConfidentialLRC20Votes public immutable CONFIDENTIAL_LRC20_VOTES;
    ...
    /// @notice Returns the proposal id associated with the request id from the TFHE.
     ...
     * @param confidentialLRC20Votes_   ConfidentialLRC20Votes token.
    ...
    constructor(
        address owner_,
        address timelock_,
        address confidentialLRC20Votes_,
        uint256 votingPeriod_,
        uint256 maxDecryptionDelay_
    ) Ownable(owner_) {
        TIMELOCK = ICompoundTimelock(timelock_);
        CONFIDENTIAL_LRC20_VOTES = IConfidentialLRC20Votes(confidentialLRC20Votes_);
    ...
    function _castVote(address voter, uint256 proposalId, ebool support) internal virtual {
        ...
        euint64 votes = CONFIDENTIAL_LRC20_VOTES.getPriorVotesForGovernor(voter, proposal.startBlock);
