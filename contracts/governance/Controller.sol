 ...
 * @dev Renamed from GuardableModule (Zodiac) to align with Lux terminology.
 *
 * The Controller pattern allows for flexible execution of transactions through a Safe,
...
abstract contract Controller {
    ...
    /// @notice Address of the vault (Safe) that this controller manages
    ...
    error GuardRejected();
    error ControllerTransactionFailed();
    ...
    modifier onlyVault() {
        require(msg.sender == vault, "Controller: caller is not the vault");
        _;
    }
    ...
    /**
     * @notice Sets the vault (Safe) address for this controller
