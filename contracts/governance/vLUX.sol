...
contract vLUX is ReentrancyGuard {
    ...
    event Deposit(address indexed user, uint256 amount, uint256 lockTime, uint256 indexed lockEnd, uint256 ts);
    ...
    constructor(address _lux) {
        ...
        lux = IERC20(_lux);
        pointHistory[0] = Point({ bias: 0, slope: 0, ts: block.timestamp, blk: block.number });
    }
    ...
    function _depositFor(
        ...
        LockedBalance memory newLocked =
            LockedBalance({ amount: oldLocked.amount + amount, end: unlockTime == 0 ? oldLocked.end : unlockTime });
