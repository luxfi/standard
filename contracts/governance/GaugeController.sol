...
contract GaugeController is ReentrancyGuard, Ownable2Step {
    ...
    constructor(address _vLux) Ownable(msg.sender) {
        ...
        gauges.push(Gauge({ recipient: address(0), name: "INVALID", gaugeType: 0, active: false }));
    }
    ...
    function addGauge(address recipient, string calldata name, uint256 gaugeType)
        external
        onlyOwner
        returns (uint256 gaugeId)
    {
        gaugeId = gauges.length;

        gauges.push(Gauge({ recipient: recipient, name: name, gaugeType: gaugeType, active: true }));
