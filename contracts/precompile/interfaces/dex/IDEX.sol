...
interface IDEX {
    ...
    event OrderFilled(
        uint64 indexed orderId, address indexed trader, uint256 amountIn, uint256 amountOut, uint256 avgPrice
    );
    ...
    function marketOrder(address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut)
        external
    ...
    function modifyOrder(uint64 orderId, uint256 newPrice, uint256 newAmount) external returns (bool success);
    ...
    function getQuote(address tokenIn, address tokenOut, uint256 amountIn) external view returns (Quote memory quote);
    ...
    function getQuotes(address tokenIn, address tokenOut, uint256[] calldata amounts)
        external
        view
    ...
    function getOrderbook(address token0, address token1, uint256 depth)
        external
        view
        returns (Orderbook memory orderbook);
    ...
    function getBestPrices(address token0, address token1)
        external
        view
        returns (uint256 bestBid, uint256 bestAsk, uint256 midPrice);

    /// @notice Get 24h volume for a pair
    /// @param token0 First token of pair
    /// @param token1 Second token of pair
    /// @return volume24h Volume in token0 terms
    function getVolume24h(address token0, address token1) external view returns (uint256 volume24h);
...
library DEXLib {
    ...
    function getPrice(address token0, address token1) internal view returns (uint256 price) {
