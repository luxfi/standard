...
library SLHDSALib {
    ...
    function verifyOrRevert(bytes memory publicKey, bytes memory message, bytes memory signature) internal view {
    ...
    function isValidPublicKeySize(bytes memory publicKey) internal pure returns (bool) {
        ...
        return len == SHA2_128_PUBKEY_SIZE || len == SHA2_192_PUBKEY_SIZE || len == SHA2_256_PUBKEY_SIZE;
    }
    ...
    function isValidSignatureSize(bytes memory signature) internal pure returns (bool) {
        ...
        return len == SHA2_128s_SIG_SIZE || len == SHA2_128f_SIG_SIZE || len == SHA2_192s_SIG_SIZE
            || len == SHA2_192f_SIG_SIZE || len == SHA2_256s_SIG_SIZE || len == SHA2_256f_SIG_SIZE;
    }
}
...
abstract contract SLHDSAVerifier {
    event SLHDSASignatureVerified(bytes32 indexed messageHash, bytes publicKey, bool valid);

    function verifySLHDSASignature(bytes memory publicKey, bytes memory message, bytes memory signature)
        internal
        view
        returns (bool valid)
    {
    ...
    function verifySLHDSASignatureWithEvent(bytes memory publicKey, bytes memory message, bytes memory signature)
        internal
        returns (bool valid)
    {
