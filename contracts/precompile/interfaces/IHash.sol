...
interface IBlake3 {
    ...
    function hashKeyed(bytes32 key, bytes calldata data) external view returns (bytes32 digest);
    ...
    function hashXOF(bytes calldata data, uint32 outputLength) external view returns (bytes memory output);
    ...
    function deriveKey(string calldata context, bytes calldata material) external view returns (bytes32 key);
    ...
    function batchHash(bytes[] calldata inputs) external view returns (bytes32[] memory digests);
}
...
interface IPoseidon2 {
    ...
    function hash2(uint256 a, uint256 b) external view returns (uint256 digest);

    /// @notice Hash 4 field elements
    function hash4(uint256 a, uint256 b, uint256 c, uint256 d) external view returns (uint256 digest);

    /// @notice Hash 8 field elements (single compression)
    function hash8(uint256[8] calldata inputs) external view returns (uint256 digest);

    /// @notice Hash array of field elements
    /// @param inputs Field elements to hash
    /// @return digest Single hash output
    function hashMany(uint256[] calldata inputs) external view returns (uint256 digest);

    /// @notice Compute merkle root with Poseidon2
    /// @param leaves Array of field element leaves
    /// @return root Merkle root
    function merkleRoot(uint256[] calldata leaves) external view returns (uint256 root);

    /// @notice Hash bytes to field element
    /// @param data Raw bytes to hash
    /// @return digest Field element hash
    function hashBytes(bytes calldata data) external view returns (uint256 digest);
}
...
interface IPoseidon2Sponge {
    ...
    function sponge(uint256[] calldata inputs, uint32 outputCount) external view returns (uint256[] memory outputs);
    ...
    function encrypt(uint256 key, uint256 nonce, uint256[] calldata plaintext)
        external
        view
        returns (uint256[] memory ciphertext);
    ...
    function decrypt(uint256 key, uint256 nonce, uint256[] calldata ciphertext)
        external
        view
        returns (uint256[] memory plaintext);
}
...
interface IPedersen {
    ...
    function hash(bytes calldata data) external view returns (uint256 x, uint256 y);
    ...
    function commit(uint256 value, uint256 blinding) external view returns (bytes memory commitment);
    ...
    function verify(bytes calldata commitment, uint256 value, uint256 blinding) external view returns (bool valid);
    ...
    function batchCommit(uint256[] calldata values, uint256[] calldata blindings)
        external
        view
        returns (bytes[] memory commitments);
    ...
    function add(bytes calldata commitment1, bytes calldata commitment2) external view returns (bytes memory sum);

    /// @notice Subtract commitments homomorphically
    /// @param commitment1 First commitment
    /// @param commitment2 Second commitment
    /// @return diff Commitment to difference
    function sub(bytes calldata commitment1, bytes calldata commitment2) external view returns (bytes memory diff);

    /// @notice Scalar multiply a commitment
    /// @param commitment Input commitment
    /// @param scalar Scalar multiplier
    /// @return result Commitment to scaled value
    function scalarMul(bytes calldata commitment, uint256 scalar) external view returns (bytes memory result);
}
...
library HashLib {
    ...
    function blake3(bytes memory data) internal view returns (bytes32) {
        (bool success, bytes memory result) = BLAKE3.staticcall(abi.encodeCall(IBlake3.hash, (data)));
        require(success, "Blake3 hash failed");
        return abi.decode(result, (bytes32));
    }

    /// @notice Hash with keyed Blake3 (MAC)
    function blake3Keyed(bytes32 key, bytes memory data) internal view returns (bytes32) {
        (bool success, bytes memory result) = BLAKE3.staticcall(abi.encodeCall(IBlake3.hashKeyed, (key, data)));
        require(success, "Blake3 keyed hash failed");
        return abi.decode(result, (bytes32));
    }

    /// @notice Derive key with Blake3
    function blake3DeriveKey(string memory context, bytes memory material) internal view returns (bytes32) {
        (bool success, bytes memory result) = BLAKE3.staticcall(abi.encodeCall(IBlake3.deriveKey, (context, material)));
        require(success, "Blake3 key derivation failed");
        return abi.decode(result, (bytes32));
    }

    /// @notice Compute Blake3 merkle root
    function blake3MerkleRoot(bytes32[] memory leaves) internal view returns (bytes32) {
        (bool success, bytes memory result) = BLAKE3.staticcall(abi.encodeCall(IBlake3.merkleRoot, (leaves)));
    ...
    function poseidon2(uint256 a, uint256 b) internal view returns (uint256) {
        (bool success, bytes memory result) = POSEIDON2.staticcall(abi.encodeCall(IPoseidon2.hash2, (a, b)));
        require(success, "Poseidon2 hash failed");
        return abi.decode(result, (uint256));
    }

    /// @notice Hash array with Poseidon2
    function poseidon2Many(uint256[] memory inputs) internal view returns (uint256) {
        (bool success, bytes memory result) = POSEIDON2.staticcall(abi.encodeCall(IPoseidon2.hashMany, (inputs)));
        require(success, "Poseidon2 hash failed");
        return abi.decode(result, (uint256));
    }

    /// @notice Compute Poseidon2 merkle root
    function poseidon2MerkleRoot(uint256[] memory leaves) internal view returns (uint256) {
        (bool success, bytes memory result) = POSEIDON2.staticcall(abi.encodeCall(IPoseidon2.merkleRoot, (leaves)));
        require(success, "Poseidon2 merkle root failed");
        return abi.decode(result, (uint256));
    }

    /// @notice Create Pedersen commitment
    function pedersenCommit(uint256 value, uint256 blinding) internal view returns (bytes memory) {
        (bool success, bytes memory result) = PEDERSEN.staticcall(abi.encodeCall(IPedersen.commit, (value, blinding)));
        require(success, "Pedersen commit failed");
        return abi.decode(result, (bytes));
    }

    /// @notice Verify Pedersen commitment
    function pedersenVerify(bytes memory commitment, uint256 value, uint256 blinding) internal view returns (bool) {
        (bool success, bytes memory result) =
            PEDERSEN.staticcall(abi.encodeCall(IPedersen.verify, (commitment, value, blinding)));
        return success && abi.decode(result, (bool));
    }

    /// @notice Add two Pedersen commitments (homomorphic)
    function pedersenAdd(bytes memory c1, bytes memory c2) internal view returns (bytes memory) {
        (bool success, bytes memory result) = PEDERSEN.staticcall(abi.encodeCall(IPedersen.add, (c1, c2)));
    ...
    function optimalHash(bytes memory data, bool zkFriendly) internal view returns (bytes32) {
        if (zkFriendly) {
            // Use Poseidon2 for ZK circuits (returns field element)
            (bool success, bytes memory result) = POSEIDON2.staticcall(abi.encodeCall(IPoseidon2.hashBytes, (data)));
            require(success, "Poseidon2 hash failed");
...
abstract contract HashConsumer {
    ...
    function computeContentHash(bytes calldata content) external view returns (bytes32) {
        return HashLib.blake3(content);
    }

    /// @notice Create authenticated message
    function createMAC(bytes32 key, bytes calldata message) external view returns (bytes32) {
        return HashLib.blake3Keyed(key, message);
    }

    /// @notice Verify merkle inclusion
    function verifyMerkleProof(bytes32 root, bytes32 leaf, bytes32[] calldata proof, uint256 index)
        external
        view
        returns (bool)
    {
...
abstract contract ConfidentialBalance {
    ...
    function proveMinBalance(uint256 claimedValue, uint256 blinding) external view returns (bool) {
