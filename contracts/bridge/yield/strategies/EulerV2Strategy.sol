...
interface IEVaultFactory {
    ...
    function createProxy(address implementation, bool upgradeable, bytes memory trailingData) external returns (address);
...
abstract contract EulerV2BaseStrategy is Ownable, ReentrancyGuard {
    ...
    function getVaultMetrics()
        external
        view
        returns (uint256 totalSupply, uint256 totalBorrows, uint256 interestRate, uint256 utilization)
    {
...
contract EulerV2USDCStrategy is EulerV2BaseStrategy {
    ...
    constructor(address _controller, address _owner) EulerV2BaseStrategy(USDC_EVAULT, _controller, _owner) { }
...
contract EulerV2WETHStrategy is EulerV2BaseStrategy {
    ...
    constructor(address _controller, address _owner) EulerV2BaseStrategy(WETH_EVAULT, _controller, _owner) { }
...
contract EulerV2WstETHStrategy is EulerV2BaseStrategy {
    ...
    constructor(address _controller, address _owner) EulerV2BaseStrategy(WSTETH_EVAULT, _controller, _owner) { }
...
contract EulerV2DAIStrategy is EulerV2BaseStrategy {
    ...
    constructor(address _controller, address _owner) EulerV2BaseStrategy(DAI_EVAULT, _controller, _owner) { }
...
contract EulerV2USDTStrategy is EulerV2BaseStrategy {
    ...
    constructor(address _controller, address _owner) EulerV2BaseStrategy(USDT_EVAULT, _controller, _owner) { }
...
contract EulerV2LeveragedStrategy is Ownable, ReentrancyGuard {
    ...
    function closePosition() external onlyController nonReentrant returns (uint256 returned) {
        ...
        if (totalBorrowed > 0) {
            ...
            if (address(borrowAsset) == address(collateralAsset)) {
                uint256 totalInVault = collateralVault.convertToAssets(collateralVault.balanceOf(address(this)));
...
contract EulerV2StrategyFactory is Ownable {
    ...
    event StrategyDeployed(address indexed strategy, address indexed vault, string strategyType);
    ...
    function deployCustomStrategy(address vault, address controller, string calldata strategyName)
        external
        onlyOwner
        returns (address)
    {
        CustomEulerV2Strategy strategy = new CustomEulerV2Strategy(vault, controller, strategyName, msg.sender);
    ...
    function deployLeveragedStrategy(
        ...
        EulerV2LeveragedStrategy strategy = new EulerV2LeveragedStrategy(
            collateralVault, borrowVault, controller, targetLeverage, msg.sender
        );
