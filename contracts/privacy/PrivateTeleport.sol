...
contract PrivateTeleport {
    ...
    event TeleportShielded(bytes32 indexed teleportId, bytes32 noteCommitment, bytes32 encryptedAmount);

    event TeleportSwapped(bytes32 indexed teleportId, bytes32 sourceAsset, bytes32 destAsset);

    event TeleportExporting(bytes32 indexed teleportId, bytes32 destChain, address recipient);

    event TeleportComplete(bytes32 indexed teleportId, bytes32 nullifierHash);

    event TeleportCancelled(bytes32 indexed teleportId, string reason);
    ...
    function initiateTeleport(
        ...
        (bytes32 sourceChain, bytes32 sourceAsset, address sender, uint256 deadline) = _verifyWarpMessage(warpMessage);
        ...
        emit TeleportInitiated(teleportId, sourceChain, destChain, commitment, sender, deadline);
     ...
     */
    function executePrivateSwap(bytes32 teleportId, bytes32 poolId, bytes32 minOutput, bytes calldata proof) external {
        ...
        bytes32 outputCommitment = zChainAMM.swapEncrypted(poolId, record.encryptedAmount, minOutput, record.recipient);
     ...
     */
    function completeTeleport(bytes32 teleportId, bytes calldata warpConfirmation) external {
        TeleportRecord storage record = teleports[teleportId];
        if (record.teleportId == bytes32(0)) revert TeleportNotFound();
    ...
    function unshieldToXChain(
        ...
        exportTxId = zNote.exportToXChain(
            ZNote.SpendProof({
                nullifier: nullifier, merkleRoot: zNote.getNoteRoot(), merkleProof: merkleProof, zkProof: rangeProof
            }),
            destinationAddress,
            amount,
            record.sourceAsset
    ...
    function privateTransferToRecipient(
        ...
        newNoteIndex = zNote.privateTransfer(
            ZNote.SpendProof({
                nullifier: nullifier, merkleRoot: zNote.getNoteRoot(), merkleProof: merkleProof, zkProof: transferProof
            }),
            ZNote.OutputNote({ commitment: recipientCommitment, encryptedNote: encryptedNote, encryptedMemo: "" })
        );
    ...
    function splitAndTransfer(
        ...
        noteIndices = zNote.splitNote(
            ZNote.SpendProof({
                nullifier: nullifier, merkleRoot: zNote.getNoteRoot(), merkleProof: merkleProof, zkProof: splitProof
            }),
            outputs
        );
    ...
    function isExpired(bytes32 teleportId) external view returns (bool) {
        TeleportRecord storage record = teleports[teleportId];
        return block.timestamp > record.deadline && record.state != TeleportState.Complete
            && record.state != TeleportState.Cancelled;
    }
     ...
     */
    function _createShieldedNote(bytes32 teleportId, bytes32 commitment, bytes32 encryptedAmount, bytes32 asset)
        internal
    {
        TeleportRecord storage record = teleports[teleportId];

        // Import from X-Chain creates a ZNote
        zNote.importFromXChain(commitment, encryptedAmount, asset, record.recipient);
     ...
     */
    function _generateTeleportId(address sender, address recipient, bytes32 commitment) internal returns (bytes32) {
        return keccak256(abi.encodePacked(sender, recipient, commitment, block.timestamp, _teleportNonce++));
    }

    /**
     * @dev Verify Warp message from X-Chain
     */
    function _verifyWarpMessage(bytes calldata warpMessage)
        internal
        view
        returns (bytes32 sourceChain, bytes32 sourceAsset, address sender, uint256 deadline)
    {
        // Call Warp precompile to verify message
        (bool success, bytes memory result) =
            WARP_PRECOMPILE.staticcall(abi.encodeWithSignature("getVerifiedWarpMessage(uint32)", 0));
     ...
     */
    function _verifySwapProof(bytes calldata proof, bytes32 commitment, bytes32 poolId) internal view returns (bool) {
     ...
     */
    function _verifyRangeProof(bytes calldata proof, bytes32 commitment) internal view returns (bool) {
     ...
     */
    function _verifyMerkleProof(bytes32[] calldata proof, bytes32 commitment) internal view returns (bool) {
    ...
    function _sendWarpExport(TeleportRecord storage record) internal {
        ...
        (bool success,) = WARP_PRECOMPILE.call(abi.encodeWithSignature("sendWarpMessage(bytes)", payload));
     ...
     */
    function _verifyWarpConfirmation(bytes calldata warpConfirmation)
        internal
        view
        returns (bytes32 teleportId, bool success)
    {
