...
contract BulletproofVerifier is IRangeProofVerifier {
    ...
    function verifySingle(bytes32 commitment, BulletproofProof calldata proof, uint8 bitLength)
        external
        view
        override
        returns (bool)
    ...
    /// @inheritdoc IRangeProofVerifier
    function verifyAggregate(AggregateProof calldata aggProof, uint8 bitLength) external view override returns (bool) {
    ...
    /// @inheritdoc IRangeProofVerifier
    function verifyCustomRange(bytes32 commitment, bytes calldata proof, uint256 minValue, uint256 maxValue)
        external
        view
        override
        returns (bool)
    ...
    /// @inheritdoc IRangeProofVerifier
    function computeCommitment(uint256 value, uint256 blinding) external view override returns (bytes32) {
    ...
    function _verifyProofStructure(BulletproofProof calldata proof, uint8 bitLength) internal pure returns (bool) {
    ...
    function _computeYZ(bytes32 commitment, bytes32 A, bytes32 S) internal pure returns (uint256 y, uint256 z) {
    ...
    function _computeX(bytes32 T1, bytes32 T2, uint256 y, uint256 z)
        internal
        pure
        returns (uint256 x, uint256 xSquared)
    {
    ...
    function _verifyPolynomialCommitment(
        ...
        bytes32 left = _pointAdd(_scalarMul(generatorG, proof.t), _scalarMul(generatorH, proof.taux));

        // Compute right side
        bytes32 right = _pointAdd(
            _scalarMul(commitment, z2),
            _pointAdd(_scalarMul(proof.T1, x), _scalarMul(proof.T2, mulmod(x, x, GROUP_ORDER)))
        );
