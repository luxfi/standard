...
import { IRangeProofVerifier } from "../interfaces/IRangeProofVerifier.sol";
...
contract PrivateBridge is IShieldedPool, AccessControl, ReentrancyGuard, Pausable {
    ...
    event CrossChainDeposit(bytes32 indexed commitment, uint256 indexed destinationChainId, address destinationBridge);

    event CrossChainClaim(bytes32 indexed commitment, uint256 indexed sourceChainId, uint256 leafIndex);

    constructor(address _rangeVerifier, bytes memory _fhePublicKey, uint256 _chainId) {
    ...
    function setDestinationBridge(uint256 chainId, address bridge) external onlyRole(DEFAULT_ADMIN_ROLE) {
        destinationBridges[chainId] = bridge;
    }

    /// @notice Update FHE public key (for key rotation)
    function updateFhePublicKey(bytes calldata newKey) external onlyRole(DEFAULT_ADMIN_ROLE) {
        fhePublicKey = newKey;
    }
    ...
    function deposit(bytes32 commitment, bytes calldata encryptedAmount)
        external
        payable
        override
        nonReentrant
    ...
    function depositToken(address token, bytes32 commitment, bytes calldata encryptedAmount)
        external
        nonReentrant
        whenNotPaused
        returns (uint256 leafIndex)
    {
        ...
        leafIndex = nextLeafIndex;
        commitments[commitment] = Commitment({
            commitment: commitment, nullifierHash: bytes32(0), leafIndex: leafIndex, timestamp: uint64(block.timestamp)
        });
        commitmentsByIndex[leafIndex] = commitment;
    ...
    function depositCrossChain(bytes32 commitment, bytes calldata encryptedAmount, uint256 destinationChainId)
        external
        payable
        nonReentrant
        whenNotPaused
        returns (uint256 leafIndex)
    {
        ...
        emit ShieldedDeposit(commitment, leafIndex, uint64(block.timestamp));
        emit CrossChainDeposit(commitment, destinationChainId, destinationBridges[destinationChainId]);
    ...
    function withdraw(WithdrawalProof calldata proof) external override nonReentrant whenNotPaused returns (bool) {
        ...
        require(_verifyRangeProof(proof.amountCommitment, proof.rangeProof), "Invalid range proof");
    ...
    function shieldedTransfer(ShieldedTransfer calldata transfer)
        external
        override
        nonReentrant
        whenNotPaused
        returns (uint256 outputLeafIndex)
    {
        ...
        emit ShieldedTransferEvent(transfer.inputNullifier, transfer.outputCommitment);
    ...
    function verifyRangeProof(bytes calldata proof, bytes32 commitment) external view returns (bool valid) {
    ...
    function _verifyRangeProof(bytes32 commitment, bytes memory proof) internal view returns (bool) {
    ...
    function _verifyTransferProof(ShieldedTransfer calldata transfer) internal view returns (bool) {
    ...
    function _verifySwapProof(bytes32 inputNullifier, bytes32 outputCommitment, bytes calldata swapProof)
        internal
        view
        returns (bool)
    {
