...
contract OmnichainLPRouter is ReentrancyGuard {
    ...
    event LiquidityAdded(
        address indexed pair, address indexed provider, uint256 amount0, uint256 amount1, uint256 liquidity
    );
    ...
    event CrossChainSwap(
        address indexed user, uint256 fromChain, uint256 toChain, address[] path, uint256 amountIn, uint256 amountOut
    );
    ...
    function addLiquidity(
        ...
        (amountA, amountB) =
            _calculateOptimalAmounts(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin);
        ...
        returns (uint256 amountToken, uint256 amountLUX, uint256 liquidity)
    {
        ...
        (amountToken, amountLUX) =
            _calculateOptimalAmounts(token, WLUX, amountTokenDesired, msg.value, amountTokenMin, amountLUXMin);
    ...
    function crossChainSwap(
        ...
        if (currentChain == targetChain) {
            // Same chain swap
            uint256[] memory amounts = swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline);
        ...
        returns (uint112 reserveA, uint112 reserveB, uint32 blockTimestampLast)
    {
        ...
        (reserveA, reserveB, blockTimestampLast) =
            tokenA == token0 ? (reserve0, reserve1, timestamp) : (reserve1, reserve0, timestamp);
    }
    ...
    function _bridgeTokens(address token, uint256 amount, uint256 targetChain, address recipient) internal {
        ...
        Bridge.Token memory fromToken = Bridge.Token({
            kind: Bridge.Type.ERC20, id: 0, chainId: block.chainid, tokenAddress: token, enabled: true
        });

        Bridge.Token memory toToken =
            Bridge.Token({ kind: Bridge.Type.ERC20, id: 0, chainId: targetChain, tokenAddress: token, enabled: true });
