...
import {IWarp as IWarpMessenger} from "@luxfi/standard/precompile/interfaces/IWarp.sol";
...
abstract contract ValidatorManager is Initializable, ContextUpgradeable, IValidatorManager {
    ...
    struct ValidatorManagerStorage {
        /// @notice The chainID associated with this validator manager.
        bytes32 _chainID;
    ...
    function initializeValidatorSet(
        ...
        // Other validation checks are done by the P-Chain when converting the chain, so are not required here.
        ...
        for (uint32 i; i < numInitialValidators; ++i) {
            ...
            // convert Chain tx ID and the index of the initial validator.
            bytes32 validationID = sha256(abi.encodePacked(conversionData.chainID, i));
        ...
        // Verify that the sha256 hash of the Chain conversion data matches with the Warp message's conversionID.
        bytes32 conversionID = ValidatorMessages.unpackChainToL1ConversionMessage(
    ...
    function _getPChainWarpMessage(uint32 messageIndex)
        internal
        view
        returns (IWarpMessenger.WarpMessage memory)
    {
        (IWarpMessenger.WarpMessage memory warpMessage, bool valid) =
