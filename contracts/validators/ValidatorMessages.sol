...
library ValidatorMessages {
    // The information that uniquely identifies a chain validation period.
    ...
    // The P-Chain signs a ChainToL1ConversionMessage that is used to verify the L1's initial validators.
    uint32 internal constant CHAIN_TO_L1_CONVERSION_MESSAGE_TYPE_ID = 0;
    ...
    /**
     * @notice Packs a ChainToL1ConversionMessage message into a byte array.
     ...
     *                                 +----------+
     *
     * @param conversionID The chain conversion ID to pack into the message.
     * @return The packed message.
     */
    function packChainToL1ConversionMessage(bytes32 conversionID)
        external
        pure
        returns (bytes memory)
    {
        return abi.encodePacked(CODEC_ID, CHAIN_TO_L1_CONVERSION_MESSAGE_TYPE_ID, conversionID);
    }

    /**
     * @notice Unpacks a byte array as a ChainToL1ConversionMessage message.
     ...
     */
    function unpackChainToL1ConversionMessage(bytes memory input)
        external
        pure
        returns (bytes32)
    {
        ...
        if (typeID != CHAIN_TO_L1_CONVERSION_MESSAGE_TYPE_ID) {
        ...
        returns (ValidationPeriod memory)
    {
        ...
        // Unpack the chainID
        {
            bytes32 chainID;
            for (uint256 i; i < 32; ++i) {
                chainID |= bytes32(uint256(uint8(input[i + index])) << (8 * (31 - i)));
            }
            validation.chainID = chainID;
            index += 32;
        }
    ...
    /**
     * @notice Unpacks a byte array as a ChainValidatorWeightMessage.
     * The message format specification is the same as the one used in above for packing.
     *
     * @param input The byte array to unpack.
