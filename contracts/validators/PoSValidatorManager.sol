...
import {ReentrancyGuardUpgradeable} from "./_compat/ReentrancyGuardUpgradeable.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct PoSValidatorManagerStorage {
        ...
        /// @notice The ID of the blockchain that submits uptime proofs. This must be a blockchain validated by the chainID that this contract manages.
    ...
    function completeEndDelegation(
        ...
        if (getValidator(delegator.validationID).status != ValidatorStatus.Completed) {
            // Unpack the Warp message
            IWarpMessenger.WarpMessage memory warpMessage = _getPChainWarpMessage(messageIndex);
