    ...
    ERC165
{
     ...
     */
    function _getFreezeVotingGovernorStorage() internal pure returns (FreezeVotingGovernorStorage storage $) {
    ...
    function initialize(
        ...
        __InitializerEventEmitter_init(
            abi.encode(owner_, freezeVotesThreshold_, freezeProposalPeriod_, parentGovernor_, lightAccountFactory_)
        );
     ...
     * @inheritdoc IFreezeVotingGovernorV1
     */
    function freezeProposalStrategy() public view virtual override returns (address) {
        FreezeVotingGovernorStorage storage $ = _getFreezeVotingGovernorStorage();
    ...
    function castFreezeVote(
        ...
        _recordFreezeVote(resolvedVoter, _aggregateFreezeVotes(resolvedVoter, votingConfigsToUse_));
    }
    ...
    function supportsInterface(bytes4 interfaceId_) public view virtual override returns (bool) {
        return interfaceId_ == type(IFreezeVotingGovernorV1).interfaceId
     ...
     */
    function _aggregateFreezeVotes(address voter_, IVotingTypes.VotingConfigVoteData[] calldata votingConfigsToUse_)
        internal
        virtual
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < votingConfigsToUse_.length;) {
            IVotingTypes.VotingConfigVoteData memory configData = votingConfigsToUse_[i];
            ...
            IVotingTypes.VotingConfig memory config = strategy.votingConfig(configData.configIndex);

            // Calculate voting weight at freeze proposal creation time
            (uint256 weight, bytes memory processedData) = IVotingWeightV1(config.votingWeight)
                .calculateWeight(voter_, $base.freezeProposalCreated, configData.voteData);

            // Check for zero weight
            if (weight == 0) {
                revert NoVotingWeight(configData.configIndex, configData.voteData);
            }

            // Record the freeze vote
            IVoteTrackerV1(config.voteTracker).recordVote($base.freezeProposalCreated, voter_, processedData);

            // Emit per-config event
            emit FreezeVoteRecorded(voter_, $base.freezeProposalCreated, weight, processedData);
