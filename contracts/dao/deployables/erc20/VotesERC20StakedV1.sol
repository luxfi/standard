...
import { IVotesERC20StakedV1 } from "../../interfaces/deployables/IVotesERC20StakedV1.sol";
    ...
    ERC165
{
     ...
     */
    function _getVotesERC20StakedStorage() internal pure returns (VotesERC20StakedStorage storage $) {
     ...
     */
    function initialize(address owner_, address stakedToken_) public virtual override initializer {
        __InitializerEventEmitter_init(abi.encode(owner_, stakedToken_));
        __ERC20_init(
            string(abi.encodePacked("Staked ", IERC20Metadata(stakedToken_).name())),
            string(abi.encodePacked("st", IERC20Metadata(stakedToken_).symbol()))
        );
        __ERC20Votes_init();
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function initialize2(uint256 minimumStakingPeriod_, address[] calldata rewardsTokens_)
        public
        virtual
        override
        reinitializer(2)
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function clock() public view virtual override(IVotesERC20StakedV1, VotesUpgradeable) returns (uint48) {
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function minimumStakingPeriod() public view virtual override returns (uint256) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function rewardsTokens() public view virtual override returns (address[] memory) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function rewardsTokenData(address token_) public view virtual override returns (uint256, uint256, uint256) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
        RewardsTokenData storage _rewardsTokenData = $.rewardsTokenDatas[token_];

        if (!_rewardsTokenData.enabled) revert InvalidRewardsToken(token_);

        return (_rewardsTokenData.rewardsRate, _rewardsTokenData.rewardsDistributed, _rewardsTokenData.rewardsClaimed);
    }

    /**
     * @inheritdoc IVotesERC20StakedV1
     */
    function distributableRewards() public view virtual override returns (uint256[] memory) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();

        uint256[] memory distributableRewards_ = new uint256[]($.rewardsTokens.length);

        for (uint256 i = 0; i < $.rewardsTokens.length;) {
            distributableRewards_[i] = _distributableRewards($.rewardsTokens[i]);
    ...
    {
        uint256[] memory distributableRewards_ = new uint256[](rewardsTokens_.length);
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function stakerData(address staker_) public view virtual override returns (uint256, uint256) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
    ...
    {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
        RewardsTokenData storage rewardsTokenData_ = $.rewardsTokenDatas[token_];

        if (!rewardsTokenData_.enabled) revert InvalidRewardsToken(token_);

        return (rewardsTokenData_.stakerRewardsRates[staker_], rewardsTokenData_.stakerAccumulatedRewards[staker_]);
    }

    /**
     * @inheritdoc IVotesERC20StakedV1
     */
    function claimableRewards(address staker_) public view virtual override returns (uint256[] memory) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();

        uint256[] memory claimableRewards_ = new uint256[]($.rewardsTokens.length);

        for (uint256 i = 0; i < $.rewardsTokens.length;) {
            claimableRewards_[i] = _claimableRewards(staker_, $.rewardsTokens[i]);
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function addRewardsTokens(address[] calldata rewardsTokens_) public virtual override onlyOwner {
        _addRewardsTokens(rewardsTokens_);
    }
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function updateMinimumStakingPeriod(uint256 newMinimumStakingPeriod_) public virtual override onlyOwner {
    ...
    function unstake(uint256 amount_) public virtual override {
        ...
        if (block.timestamp < stakerData_.lastStakeTimestamp + $.minimumStakingPeriod) revert MinimumStakingPeriod();
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function distributeRewards(address[] calldata tokens_) public virtual override {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
     ...
     * @inheritdoc IVotesERC20StakedV1
     */
    function claimRewards(address recipient_, address[] calldata tokens_) public virtual override {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
    ...
    function supportsInterface(bytes4 interfaceId_) public view virtual override returns (bool) {
        return interfaceId_ == type(IVotesERC20StakedV1).interfaceId || interfaceId_ == type(IERC20).interfaceId
            || interfaceId_ == type(IVotes).interfaceId || interfaceId_ == type(IVersion).interfaceId
     ...
     */
    function _claimRewards(address _claimer, address _recipient, address _token) internal virtual {
    ...
    function _distributeRewards(address token_) internal virtual {
        ...
        uint256 newRewardsRate = token.rewardsRate + (amountToDistribute * PRECISION) / $.totalStaked;
     ...
     */
    function _addRewardsTokens(address[] calldata rewardsTokens_) internal virtual {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
    ...
    function _accumulateRewards(address staker_) internal virtual {
        ...
        for (uint256 i = 0; i < $.rewardsTokens.length;) {
            ...
            token.stakerAccumulatedRewards[
                    staker_
                ] += ($.stakerData[staker_].stakedAmount * (token.rewardsRate - token.stakerRewardsRates[staker_]))
                / PRECISION;
     ...
     */
    function _updateMinimumStakingPeriod(uint256 newMinimumStakingPeriod_) internal virtual {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
     ...
     */
    function _distributableRewards(address token_) internal view virtual returns (uint256) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
        ...
        if (token_ == NATIVE_ASSET) {
            ...
            thisBalance = IERC20(token_).balanceOf(address(this)) - $.totalStaked;
        } else {
        ...
        return thisBalance + _rewardsTokenData.rewardsClaimed - _rewardsTokenData.rewardsDistributed;
    }
     ...
     */
    function _claimableRewards(address staker_, address token_) internal view virtual returns (uint256) {
        VotesERC20StakedStorage storage $ = _getVotesERC20StakedStorage();
        RewardsTokenData storage token = $.rewardsTokenDatas[token_];

        return token.stakerAccumulatedRewards[staker_]
            + (($.stakerData[staker_].stakedAmount * (token.rewardsRate - token.stakerRewardsRates[staker_]))
                / PRECISION);
    }
}
