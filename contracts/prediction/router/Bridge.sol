...
contract Bridge is Ownable, ReentrancyGuard, IERC1155Receiver {
    ...
    event ResolutionSynced(bytes32 indexed conditionId, uint256[] payouts);
     ...
     */
    constructor(address _claims, string memory uri_) Ownable(msg.sender) {
    ...
    function lockAndBridge(
        ...
        // Generate lock ID
        lockId = keccak256(abi.encodePacked(thisChainId, destChainId, msg.sender, lockNonce++));

        // Transfer positions to this contract (lock)
        localClaims.safeBatchTransferFrom(msg.sender, address(this), positionIds, amounts, "");
    ...
    function _handleBridgePositions(bytes32 sourceChainId, bytes memory payload) internal {
        (, // MessageType
            bytes32 lockId,, // sourceChainId (already have it)
            address sourceCTF,
            bytes32 conditionId,
            address recipient,
            uint256[] memory positionIds,
            uint256[] memory amounts
        ) = abi.decode(payload, (MessageType, bytes32, bytes32, address, bytes32, address, uint256[], uint256[]));
    ...
    function _handleUnlockPositions(bytes memory payload) internal {
        ...
        localClaims.safeBatchTransferFrom(address(this), recipient, lock.positionIds, lock.amounts, "");
    ...
    function redeemResolved(
        ...
        localClaims.redeemPositions(collateralToken, parentCollectionId, lock.conditionId, indexSets);
    ...
    function cancelBridge(bytes32 lockId) external nonReentrant {
        ...
        localClaims.safeBatchTransferFrom(address(this), msg.sender, lock.positionIds, lock.amounts, "");
     ...
     */
    function _getWrappedPositionId(bytes32 sourceChainId, address sourceCTF, uint256 sourcePositionId)
        internal
        pure
        returns (uint256)
    {
        return uint256(keccak256(abi.encodePacked(sourceChainId, sourceCTF, sourcePositionId)));
    }
