...
contract VesterCap is ReentrancyGuard, Governable {
    ...
    function syncFeeGmxTrackerBalance(address _account) external nonReentrant onlyGov {
        ...
        uint256 amountToTransfer = feeGmxTrackerBalance - stakedAmount;
    ...
    function _updateBnGmxForAccount(address _account) internal {
        ...
        uint256 totalBnGmxAmount = stakedBnGmxAmount + claimableBnGmxAmount + bnGmxBalance;
        ...
        if (esGmxToMint > 0) {
            ...
            IVester(gmxVester).setBonusRewards(_account, bonusReward + esGmxToMint);
        }

        uint256 baseStakedAmount = IRewardTracker(stakedGmxTracker).stakedAmounts(_account);
        uint256 maxAllowedBnGmxAmount = baseStakedAmount * maxBoostBasisPoints / BASIS_POINTS_DIVISOR;

        if (stakedBnGmxAmount <= maxAllowedBnGmxAmount) {
            return;
        }

        uint256 amountToUnstake = stakedBnGmxAmount - maxAllowedBnGmxAmount;
        uint256 feeGmxTrackerBalance = IERC20(feeGmxTracker).balanceOf(_account);
        ...
        if (amountToUnstake > feeGmxTrackerBalance) {
            uint256 amountToUnvest = amountToUnstake - feeGmxTrackerBalance;
