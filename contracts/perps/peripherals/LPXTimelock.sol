...
import "./interfaces/ILPXTimelock.sol";
...
contract LPXTimelock is ILPXTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "LPXTimelock: forbidden");
        _;
    }

    modifier onlyAdminOrHandler() {
        require(msg.sender == admin || isHandler[msg.sender], "LPXTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "LPXTimelock: forbidden");
        _;
    }

    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, "LPXTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "LPXTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "LPXTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        require(_target != address(this), "LPXTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "LPXTimelock: invalid _buffer");
        require(_buffer > buffer, "LPXTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }
    ...
    function setFees(
        ...
        require(_taxBasisPoints < MAX_FEE_BASIS_POINTS, "LPXTimelock: invalid _taxBasisPoints");
        require(_stableTaxBasisPoints < MAX_FEE_BASIS_POINTS, "LPXTimelock: invalid _stableTaxBasisPoints");
        require(_mintBurnFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LPXTimelock: invalid _mintBurnFeeBasisPoints");
        require(_swapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LPXTimelock: invalid _swapFeeBasisPoints");
        require(_stableSwapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LPXTimelock: invalid _stableSwapFeeBasisPoints");
    ...
    function setTokenConfig(
        ...
        require(_minProfitBps <= 500, "LPXTimelock: invalid _minProfitBps");

        IVault vault = IVault(_vault);
        require(vault.whitelistedTokens(_token), "LPXTimelock: token not yet whitelisted");
    ...
    function setInPrivateTransferMode(address _token, bool _inPrivateTransferMode) external onlyAdmin {
        if (excludedTokens[_token]) {
            ...
            require(_inPrivateTransferMode == false, "LPXTimelock: invalid _inPrivateTransferMode");
        }
    ...
    function signalRedeemLpusd(address _vault, address _token, uint256 _amount) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemLpusd", _vault, _token, _amount));
        _setPendingAction(action);
        emit SignalRedeemLpusd(_vault, _token, _amount);
    }

    function redeemLpusd(address _vault, address _token, uint256 _amount) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemLpusd", _vault, _token, _amount));
        _validateAction(action);
        _clearAction(action);

        address lpusd = IVault(_vault).lpusd();
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "LPXTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "LPXTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "LPXTimelock: action time not yet passed");
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "LPXTimelock: invalid _action");
