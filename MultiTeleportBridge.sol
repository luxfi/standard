...
// File: contracts/MultiTeleportBridge.sol
...
contract MultiTeleportBridge is Ownable, AccessControl {
    ...
    event VaultReleased(address recipient, address token, uint256 amt);
    event AdminGranted(address to);
    event AdminRevoked(address to);
    ...
    /* Set fee payout addresses and fee - set at contract launch - in wei */
    function setPayoutAddress(address addr, uint256 feeR) public onlyAdmin{
        payoutAddr = addr;
        feeRate = feeR;
    }

    /* ERC20 WrappedAsset token balance for user */
    ...
    /* Map MPCOracle address at blockHeight */
    ...
    /* Used to set a new MPC address at block height - only MPC signers can update */
    ...
    /* For tokens that don't allow users to burn */
    ...
    function bridgeMintStealth(uint256 amt, string memory hashedId, string memory toTargetAddrStr, bytes memory signedTXInfo, uint bhUint, string memory tokenAddrStr, uint256 chainId, bool vault) public returns (address) { //chainId
        ...
        emit BridgeMinted(toTargetAddr, varStruct.tokenAddr, amt);
    ...
    function unvaultStealth(uint256 amt, string memory hashedId, string memory toTargetAddrStr, bytes memory signedTXInfo, uint bhUint, string memory tokenAddrStr, uint256 chainId, bool vault) public returns (address) { 
        ...
        emit VaultReleased(toTargetAddr, varStruct.tokenAddr, amt);
        return signer;
    }
