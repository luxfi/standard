...
%==============================================================================
\section{Liquid Protocol}
%==============================================================================

\subsection{Overview}

The Liquid Protocol provides 1:1 liquid tokens for bridged collateral and self-repaying loans:
\begin{enumerate}
    \item Bridge assets (ETH, BTC, USDC) to Lux Network
    \item Deposit as collateral via Teleport to mint liquid tokens (LETH, LBTC, LUSD)
    \item Liquid tokens accrue yield from external strategies
    \item Debt self-repays as collateral generates yield
\end{enumerate}

\subsection{Token Architecture}

\textbf{Bridged Assets} arrive on Lux as their native symbol (ETH, BTC, USDC, USDT).

\textbf{Liquid Tokens (L* prefix)} are minted 1:1 when bridged assets are deposited as collateral:
\begin{itemize}
    \item \textbf{LETH}: Liquid ETH (1:1 for deposited ETH)
    \item \textbf{LBTC}: Liquid BTC (1:1 for deposited BTC)
    \item \textbf{LUSD}: Liquid USD (1:1 for deposited USDC/USDT)
    \item \textbf{LSOL}: Liquid SOL (1:1 for deposited SOL)
    \item \textbf{LTON}: Liquid TON (1:1 for deposited TON)
\end{itemize}

\textbf{Liquid Staking (xLUX)}:
\begin{itemize}
    \item \textbf{xLUX}: LiquidLUX---master yield vault for staked LUX
    \item Receives all protocol fees (DEX, bridge, lending, perps)
    \item Only native LUX has liquid staking (xLUX)
\end{itemize}

\subsection{Zoo Network (Z* prefix)}

Same model on Zoo Network:
\begin{itemize}
    \item \textbf{ZETH, ZBTC, ZUSD}: Liquid tokens on Zoo
    \item \textbf{xZOO}: LiquidZOO for native ZOO staking
\end{itemize}
...
\texttt{sLUX} & Staked LUX token \\
\texttt{LiquidLUX} & Master yield vault (xLUX) \\
\bottomrule
\end{tabular}
