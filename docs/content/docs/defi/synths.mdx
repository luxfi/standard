...
- **Multiple Synthetics**: sUSD, sETH, sBTC, sLUX, and more
...
│         ▼                  ▼                    ▼                   ▼                  │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐          │
│  │ YieldToken  │     │  sUSD/sETH  │     │  Strategy   │     │ Transmuter  │          │
...
|-------|------|-----------|----------|
| **sUSD** | Lux Synthetic USD | LUSD, yield vaults | Stablecoin borrowing |
| **sETH** | Lux Synthetic ETH | LETH, staked ETH | Leveraged ETH exposure |
| **sBTC** | Lux Synthetic BTC | LBTC, wrapped BTC | Bitcoin borrowing |
| **sLUX** | Lux Synthetic LUX | sLUX, staked LUX | LUX leverage |
| **sAI** | Lux Synthetic AI | AI, staked AI | AI token borrowing |
| **sZOO** | Lux Synthetic ZOO | LZOO | Zoo token borrowing |
...
IERC20(sETH).approve(address(alchemist), burnAmount);
...
```solidity
import "@luxfi/contracts/synths/TransmuterV2.sol";

TransmuterV2 transmuter = TransmuterV2(TRANSMUTER_ADDRESS);

// Deposit synths for redemption
IERC20(sUSD).approve(address(transmuter), amount);
...
// 2. User mints 5,000 sETH (50% LTV)
...
```solidity
import "@luxfi/contracts/synths/interfaces/IERC3156FlashLender.sol";

contract FlashBorrower is IERC3156FlashBorrower {
    function executeFlashLoan(uint256 amount) external {
        IERC3156FlashLender(sUSD).flashLoan(
            this,
            address(sUSD),
            amount,
            ""
