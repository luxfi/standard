...
contract("LamportTest test", (accounts: string[]) => {
  it("can broadcast message via broadcast2", async () => {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      const current_keys: LamportKeyPair = JSON.parse(
        JSON.stringify(k.currentKeyPair())
      );
      const next_keys: LamportKeyPair = JSON.parse(
        JSON.stringify(k.getNextKeyPair())
      );
      ...
      const packed = ethers.utils.solidityPack(
        ["string", "bytes[2][256]"],
        [messageToBroadcast, next_keys.pub]
      );
    ...
    fs.writeFileSync(
      "gas_data.json",
      JSON.stringify(gas_data, null, 2),
      "utf8"
    );
  });
  ...
  it("cannot broadcast if signature is altered", async () => {
    ...
    const altered_sig: Sig = sig.map((s, i) => {
      if (i === 0) return "0".repeat(s.length);
      return s;
    });
  ...
  it("can move to new tip jar", async () => {
    ...
    const packed = ethers.utils.solidityPack(
      ["address", "bytes[2][256]"],
      [wallet_newTipJar.address, next_keys.pub]
    );
    const callhash = hash_b(packed);
    const sig = sign_hash(callhash, current_keys.pri);
    const is_valid_sig = verify_signed_hash(callhash, sig, current_keys.pub);
    ...
    await _contract.change_tip_jar(
      wallet_newTipJar.address,
      next_keys.pub,
      sig.map((s) => `0x${s}`),
      { from: accounts[0] }
    );
...
const pubFromPri = (pri: [string, string][]) =>
  pri.map((p) => [hash_b(p[0]), hash_b(p[1])]) as PubPair[];
...
function mk_key_pair(): KeyPair {
  ...
  const mk_pri_key = () =>
    Array.from({ length: 256 }, () => mk_RandPair()) as RandPair[];
...
function sign_hash(hmsg: string, pri: RandPair[]): Sig {
  if (!is_private_key(pri)) throw new Error("invalid private key");

  const msg_hash_bin = new BigNumber(hmsg, 16).toString(2).padStart(256, "0");

  if (msg_hash_bin.length !== 256)
    throw new Error(
      `invalid message hash length: ${msg_hash_bin.length} --> ${msg_hash_bin}`
    );

  const sig: Sig = ([...msg_hash_bin] as ("0" | "1")[]).map(
    (el: "0" | "1", i: number) => pri[i][el]
  );
  return sig;
}
...
function verify_signed_hash(hmsg: string, sig: Sig, pub: PubPair[]): boolean {
  const msg_hash_bin = new BigNumber(hmsg, 16).toString(2).padStart(256, "0");
  const pub_selection = ([...msg_hash_bin] as ("0" | "1")[]).map(
    (
      way: /** 'way' as in which way should we go through the public key */
        | "0"
        | "1",
      i: number
    ) => pub[i][way]
  ...
  for (let i = 0; i < pub_selection.length; i++)
    if (pub_selection[i] !== hash_b(`0x${sig[i]}`)) return false;
...
class KeyTracker {
  ...
  currentKeyPair(): LamportKeyPair {
    if (this.privateKeys.length == 0) return this.getNextKeyPair();
    return {
  ...
  previousKeyPair(): LamportKeyPair {
    if (this.privateKeys.length < 2) throw new Error("no previous key pair");
    return {
      pri: this.privateKeys[this.privateKeys.length - 2],
      pub: this.publicKeys[this.publicKeys.length - 2],
    };
  }
}
