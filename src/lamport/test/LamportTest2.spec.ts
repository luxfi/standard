  ...
  it("can broadcast message via broadcast2", async () => {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
...

      {
        ...
        expect(KeyTracker.pkhFromPublicKey(current_keys.pub)).to.deep.equal(
          expectedPKH
        );
      }
      ...
      const packed_old = ethers.utils.solidityPack(
        ["string", "bytes32"],
        [messageToBroadcast, nextpkh]
      );
      const packed = (() => {
        const temp = ethers.utils.solidityPack(
          ["string"],
          [messageToBroadcast]
        );
  ...
  it("broadcastWithNumber", async () => {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      const packed = (() => {
        const temp = ethers.utils.solidityPack(
          ["string", "uint256"],
          [messageToBroadcast, numToBroadcast]
        );
        return ethers.utils.solidityPack(["bytes", "bytes32"], [temp, nextpkh]);
      })();
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      const packed = (() => {
        const temp = ethers.utils.solidityPack(
          ...
          [messageToBroadcast, numToBroadcast, addressToBroadcast]
        );
        return ethers.utils.solidityPack(["bytes", "bytes32"], [temp, nextpkh]);
      })();
