...
const calculateSafeDomainSeparator = (safeAddress, chainId) => {
    return ethers_1.ethers.TypedDataEncoder.hashDomain({ verifyingContract: safeAddress, chainId });
};
exports.calculateSafeDomainSeparator = calculateSafeDomainSeparator;
const preimageSafeTransactionHash = (safeAddress, safeTx, chainId) => {
    return ethers_1.ethers.TypedDataEncoder.encode({ verifyingContract: safeAddress, chainId }, exports.EIP712_SAFE_TX_TYPE, safeTx);
};
exports.preimageSafeTransactionHash = preimageSafeTransactionHash;
const calculateSafeTransactionHash = (safeAddress, safeTx, chainId) => {
    return ethers_1.ethers.TypedDataEncoder.hash({ verifyingContract: safeAddress, chainId }, exports.EIP712_SAFE_TX_TYPE, safeTx);
};
exports.calculateSafeTransactionHash = calculateSafeTransactionHash;
const preimageSafeMessageHash = (safeAddress, message, chainId) => {
    return ethers_1.ethers.TypedDataEncoder.encode({ verifyingContract: safeAddress, chainId }, exports.EIP712_SAFE_MESSAGE_TYPE, { message });
};
exports.preimageSafeMessageHash = preimageSafeMessageHash;
const calculateSafeMessageHash = (safeAddress, message, chainId) => {
    return ethers_1.ethers.TypedDataEncoder.hash({ verifyingContract: safeAddress, chainId }, exports.EIP712_SAFE_MESSAGE_TYPE, { message });
};
...
const safeApproveHash = async (signer, safe, safeTx, skipOnChainApproval) => {
    if (!skipOnChainApproval) {
        ...
        const typedDataHash = (0, exports.calculateSafeTransactionHash)(safeAddress, safeTx, chainId);
...
const safeSignTypedData = async (signer, safeAddress, safeTx, chainId) => {
    ...
    return {
        signer: signerAddress,
        data: await signer.signTypedData({ verifyingContract: safeAddress, chainId: cid }, exports.EIP712_SAFE_TX_TYPE, safeTx),
    };
};
exports.safeSignTypedData = safeSignTypedData;
const signHash = async (signer, hash) => {
    const typedDataHash = ethers_1.ethers.getBytes(hash);
    const signerAddress = await signer.getAddress();
    return {
        signer: signerAddress,
...
const safeSignMessage = async (signer, safeAddress, safeTx, chainId) => {
    const cid = chainId || (await signer.provider.getNetwork()).chainId;
    return (0, exports.signHash)(signer, (0, exports.calculateSafeTransactionHash)(safeAddress, safeTx, cid));
};
...
const logGas = async (message, tx, skip) => {
    return tx.then(async (result) => {
        ...
        if (!skip)
            console.log("           Used", receipt.gasUsed, `gas for >${message}<`);
        return result;
    });
};
...
const buildContractCall = async (contract, method, params, nonce, delegateCall, overrides) => {
...
const executeTxWithSigners = async (safe, tx, signers, overrides) => {
    const safeAddress = await safe.getAddress();
    const sigs = await Promise.all(signers.map((signer) => (0, exports.safeSignTypedData)(signer, safeAddress, tx)));
...
const executeContractCallWithSigners = async (safe, contract, method, params, signers, delegateCall, overrides) => {
    const tx = await (0, exports.buildContractCall)(contract, method, params, await safe.nonce(), delegateCall, overrides);
