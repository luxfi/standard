...
const encodeMetaTransaction = (tx) => {
    const data = ethers_1.ethers.getBytes(tx.data);
    const encoded = ethers_1.ethers.solidityPacked(["uint8", "address", "uint256", "uint256", "bytes"], [tx.operation, tx.to, tx.value, data.length, data]);
...
const buildMultiSendSafeTx = async (multiSend, txs, nonce, overrides) => {
