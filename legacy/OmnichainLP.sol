...
contract OmnichainLP is ERC20, IERC20Bridgable, Ownable, ReentrancyGuard {
    ...
    function addLiquidity(
        ...
        chainLiquidity[block.chainid] = chainLiquidity[block.chainid].add(liquidity);
        userChainBalances[to][block.chainid] = userChainBalances[to][block.chainid].add(liquidity);
    ...
    function removeLiquidity(
        ...
        amount0 = liquidity.mul(reserve0) / _totalSupply;
        ...
        // Update chain liquidity tracking
        chainLiquidity[block.chainid] = chainLiquidity[block.chainid].sub(liquidity);
        userChainBalances[msg.sender][block.chainid] = userChainBalances[msg.sender][block.chainid].sub(liquidity);
    ...
    function bridgeLPTokens(
        ...
        // Update chain liquidity tracking
        chainLiquidity[block.chainid] = chainLiquidity[block.chainid].sub(amount);
        userChainBalances[msg.sender][block.chainid] = userChainBalances[msg.sender][block.chainid].sub(amount);
    ...
    function bridgeMint(address to, uint256 amount) external override onlyBridge {
        _mint(to, amount);
        chainLiquidity[block.chainid] = chainLiquidity[block.chainid].add(amount);
        userChainBalances[to][block.chainid] = userChainBalances[to][block.chainid].add(amount);
    ...
    function bridgeBurn(address from, uint256 amount) external override onlyBridge {
        _burn(from, amount);
        chainLiquidity[block.chainid] = chainLiquidity[block.chainid].sub(amount);
        userChainBalances[from][block.chainid] = userChainBalances[from][block.chainid].sub(amount);
        emit CrossChainSync(block.chainid, chainLiquidity[block.chainid]);
    }
