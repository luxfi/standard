...
contract PerpsTest is Test {
    ...
    function setUp() public {
        ...
        // Deploy LLPManager
        llpManager = new LLPManager(address(vault), address(lpusd), address(llp), address(shortsTracker), 15 minutes);
    ...
    function _configurePriceFeeds() internal {
        ...
        vaultPriceFeed.setTokenConfig(address(wbtc), address(btcPriceFeed), 8, false);
    ...
    function _configureTokens() internal {
        ...
        // Configure WBTC
        vault.setTokenConfig(address(wbtc), 8, 10000, 75, 0, false, true);
    ...
    function test_RemoveLiquidity() public {
        ...
        vm.startPrank(alice);
        weth.approve(address(llpManager), depositAmount);
        uint256 llpAmount = llpManager.addLiquidity(address(weth), depositAmount, 0, 0);
    ...
    function test_OnlyGovCanSetFees() public {
        vm.prank(alice);
        vm.expectRevert();
        vault.setFees(50, 20, 30, 30, 4, 10, 5e30, 3 hours, true);
    ...
    function testFuzz_AddRemoveLiquidity(uint256 amount) public {
        ...
        uint256 llpAmount = llpManager.addLiquidity(address(weth), amount, 0, 0);
...
contract PerpsEdgeCaseTest is PerpsTest {
    function test_CannotInitializeVaultTwice() public {
        vm.expectRevert();
        vault.initialize(address(0), address(lpusd), address(vaultPriceFeed), 5e30, 100, 100);
    }
    ...
    function test_CannotSwapToUnwhitelistedToken() public {
        ...
        vm.expectRevert();
        router.swap(new address[](2), 1e18, 0, alice);
        vm.stopPrank();
    }
